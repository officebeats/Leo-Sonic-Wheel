...
const PLAYER_BODY = "#2980E8";
const PLAYER_BELLY = "#F5DEB3";
const PLAYER_WHEEL = "#B0BEC5";
...
function drawPlayer(ctx: CanvasRenderingContext2D, gs: GameState) {
  ...
  // wheel (circle behind character)
  const wheelRadius = 12;
  const wheelCx = x + PLAYER_WIDTH / 2;
  const wheelCy = y + PLAYER_HEIGHT - wheelRadius + 2;
  const wheelAngle = gs.frameCount * gs.vx * 0.08;

  ctx.save();
  ctx.translate(wheelCx, wheelCy);
  ctx.rotate(wheelAngle);
  ctx.strokeStyle = PLAYER_WHEEL;
  ctx.lineWidth = 2.5;
  ctx.beginPath();
  ...
  const bodyW = 18;
  const bodyH = 16;
  const bodyX = x + (PLAYER_WIDTH - bodyW) / 2;
  const bodyY = y + 2;
  ctx.beginPath();
  ctx.ellipse(
    bodyX + bodyW / 2,
  ...
  // belly
  ctx.fillStyle = PLAYER_BELLY;
  ctx.beginPath();
  ctx.ellipse(
    bodyX + bodyW / 2 + (gs.facingRight ? 2 : -2),
    bodyY + bodyH / 2 + 2,
    5,
    6,
    0,
  ...
  // eye
  ctx.fillStyle = "#FFF";
  const eyeX = bodyX + (gs.facingRight ? bodyW - 6 : 3);
  ctx.beginPath();
  ctx.arc(eyeX, bodyY + 5, 4, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#111";
  ctx.beginPath();
  ctx.arc(eyeX + (gs.facingRight ? 1 : -1), bodyY + 5, 2, 0, Math.PI * 2);
  ctx.fill();

  // spikes (hedgehog quills)
  ctx.fillStyle = "#1A5FBF";
  const spikeDir = gs.facingRight ? -1 : 1;
  for (let i = 0; i < 3; i++) {
    const sx = bodyX + bodyW / 2 + spikeDir * (bodyW / 2 - 2);
    const sy = bodyY + 2 + i * 5;
    ctx.beginPath();
    ctx.moveTo(sx, sy);
    ctx.lineTo(sx + spikeDir * 8, sy + 1);
    ctx.lineTo(sx, sy + 4);
    ctx.fill();
  }
}
